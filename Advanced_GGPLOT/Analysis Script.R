...
Quebec %>% group_by(PriceRange) %>% summarise(sem= sd(stars)/sqrt(length(stars)), stars=mean(stars)) %>% 
  ggplot(aes(PriceRange, stars)) + geom_bar(stat = "identity", position="dodge") + theme_minimal()
...
Quebec %>% group_by(PriceRange) %>% summarise(sem= sd(stars)/sqrt(length(stars)), stars=mean(stars)) %>% 
  ggplot(aes(PriceRange, stars)) + geom_bar(stat = "identity", position="dodge") + geom_errorbar(aes(ymin=stars-sem, ymax=stars+sem), width=0, position="dodge") + theme_minimal()
...
Quebec %>% group_by(PriceRange) %>% summarise(sem= sd(stars)/sqrt(length(stars)), stars=mean(stars)) %>% 
  ggplot(aes(PriceRange, stars)) + geom_point() + geom_errorbar(aes(ymin=stars-sem, ymax=stars+sem), width=0) + geom_smooth(method="lm", se=FALSE, color="hotpink") + theme_minimal()
...
Quebec %>% filter(city %in% c("MONTREAL", "LAVAL", "WESTMOUNT")) %>%
  ggplot(aes(PriceRange, stars, color=city)) + geom_point() + scale_color_manual(values=c("deepskyblue", "hotpink1", "black")) + theme_minimal()
...
Quebec %>% filter(city %in% c("MONTREAL", "LAVAL", "WESTMOUNT")) %>%
  ggplot(aes(PriceRange, stars, color=PriceRange)) + geom_point() +ggtitle("TITLE") + xlab("X") + ylab("Y") +theme(plot.title = element_text(hjust=0.5))
...
Quebec %>% filter(city %in% c("MONTREAL", "LAVAL", "WESTMOUNT")) %>%
  ggplot(aes(PriceRange, stars, color=PriceRange)) + geom_point() +
  ggtitle("TITLE") + theme(panel.background = element_rect(fill="hotpink1"))
...
#### Graph Inserts #######
plot2 <- Quebec %>% group_by(PriceRange) %>% summarise(sem= sd(stars)/sqrt(length(stars)), stars=mean(stars)) %>% 
  ggplot(aes(PriceRange, stars)) + geom_bar(stat = "summary", position="dodge") + geom_errorbar(aes(ymin=stars-sem, ymax=stars+sem), width=0, position="dodge") + theme_minimal() + geom_hline(yintercept=3) + theme(plot.background = element_rect(colour = "black"))
